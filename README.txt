...
Using the OmnisChain.zip package:

Unpack the OmnisChain.zip package and open a shell in the resulting nxt
...
message "OMN server 1.x.x started successfully" in the console log. If run in
desktop mode, a JavaFX window will open automatically. Otherwise, open a
browser, without stopping the java process, and go to http://localhost:6969 ,
where the OMN UI should now be available.
...
There are many ways to contribute to OMN. Here are some examples:
...
The java process communicates with peers on port 6974 tcp by default. If you are
...
The user interface is available on port 6969. This port also accepts http API
requests which other OMN client applications could use.
...
The default OMN client does not store any wallet-type file on disk. Unlike
...
In addition to the default user interface at http://localhost:6969 , the
following urls are available:

http://localhost:6969/test - a list of all available http API requests, very
...
http://localhost:6969/test?requestType=<specificRequestType> - same as above,
but only shows the form for the request type specified.

http://localhost:6969/doc - a javadoc documentation for client developers who
