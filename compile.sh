...
echo "compiling OMN core..."
find src/java/nxt/ -name "*.java" > sources.tmp
javac -encoding utf8 -sourcepath "${SP}" -classpath "${CP}" -d classes/ @sources.tmp || exit 1
echo "OMN core class files compiled successfully"

echo "compiling OMN desktop..."
find src/java/nxtdesktop/ -name "*.java" > sources.tmp
javac -encoding utf8 -sourcepath "${SP}" -classpath "${CP}" -d classes/ @sources.tmp
if [ $? -eq 0 ]; then
    echo "OMN desktop class files compiled successfully"
else
    echo "if javafx is not supported, OMN desktop compile errors are safe to ignore, but desktop wallet will not be available"
fi
